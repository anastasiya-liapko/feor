...
$(function () {
    window.map_templatePlace = function (markerElem, coordinates) {
        ...
        if (person === '') {
            templateElement.querySelector('.popup__person').classList.remove('d-flex')
            templateElement.querySelector('.popup__person').classList.add('d-none')
        } else {
            templateElement.querySelector('.popup__person .popup__text').textContent = person;
        }
